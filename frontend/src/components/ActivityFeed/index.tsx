...
export const ActivityFeed: React.FC<ActivityFeedProps> = ({ projectId }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-elevated/40 border border-line rounded-xl overflow-hidden">
      {/* Feed Card Header */}
      <div className="p-4 border-b border-line bg-surface-elevated flex items-center justify-between">
        <h3 className="font-semibold text-sm tracking-wide text-fg uppercase">
          {isTeamProject
            ? '🎯 Team Collaboration Stream'
            : '📝 Project Context & Notes'}
        </h3>
        <span className="text-xs bg-surface/60 text-fg/80 px-2 py-1 rounded-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {sortedFeed.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-fg/70 text-sm py-8 text-center">
      ...
      <form
        ...
        className="p-3 border-t border-line bg-surface-elevated"
      >
        <input
          type="text"
          value={inputValue}
          ...
          className="w-full px-3 py-2 bg-surface border border-line rounded-lg text-sm focus:outline-none focus:border-highlight text-fg placeholder-fg-muted"
        />
      </form>
    </div>
  );
